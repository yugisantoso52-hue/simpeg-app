...
powershell -Command "try { $r = Invoke-RestMethod -Uri 'https://sikap.fkp.unri.ac.id/api/deploy-webhook?key=sikap_deploy_sec_2026_unri' -TimeoutSec 45; Write-Host ('[SERVER RESPONSE] ' + $r.message) -ForegroundColor Green } catch { Write-Host ('[SERVER NOTICE] Sinyal terkirim. Server sedang memproses pembaruan...') -ForegroundColor Yellow }"
