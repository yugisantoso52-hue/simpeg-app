<x-app-layout>
    <x-slot name="header">
        <x-enterprise.page-header
            ...
            subtitle="Kelola data master jenis jabatan kepegawaian (bidang keperawatan & spesialisasi)">
            <a href="{{ route('jenis-jabatan.create') }}"
            ...
            <x-enterprise.card>
                <div class="p-6">
                    <x-enterprise.toolbar
                        :create="route('jenis-jabatan.create')"
                        :searchAction="route('jenis-jabatan.index')"
                        :searchValue="request('search')"
                        placeholder="Cari Jenis Jabatan..."
                        createLabel="Tambah Jenis Jabatan"
                    />
                    ...
                    <div class="mt-6">
                        <x-enterprise.data-table>
                            <x-slot name="head">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left">Nama Jenis Jabatan</th>
                                    <th class="px-4 py-3 text-left">Keterangan</th>
                                    <th class="px-4 py-3 text-center whitespace-nowrap">Aksi</th>
                                </tr>
                            </x-slot>

                            @forelse($jenisJabatan as $row)
                                <tr class="hover:bg-slate-50 transition">
                                    <td class="px-4 py-3 text-center font-medium">
                                        @if(method_exists($jenisJabatan, 'currentPage'))
                                            {{ ($jenisJabatan->currentPage() - 1) * $jenisJabatan->perPage() + $loop->iteration }}
                                        @else
                                            {{ $loop->iteration }}
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 font-medium text-slate-900">
                                        {{ $row->nama_jenis_jabatan }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-center whitespace-nowrap">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('jenis-jabatan.edit', $row->id) }}"
                                            ...
                                            <form action="{{ route('jenis-jabatan.destroy', $row->id) }}" method="POST" class="inline-block m-0 p-0">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus Jenis Jabatan ini?')"
                                ...
                                <tr>
                                    <td colspan="4">
                                        <x-enterprise.empty-state
                                            title="Belum ada data jenis jabatan"
                                            description="Silakan tambahkan data jenis jabatan terlebih dahulu."
                                        />
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        @if(method_exists($jenisJabatan, 'links'))
                            <div class="mt-4">
                                {{ $jenisJabatan->links() }}
                            </div>
                        @endif
