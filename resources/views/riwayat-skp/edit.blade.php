        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    <span>📊</span> Edit Arsip SKP Tahun {{ $data->tahun }}
                </h2>
                <p class="text-sm text-gray-500 mt-1">
                    Perbarui predikat kinerja atau ganti berkas rencana / evaluasi SKP
                </p>
            </div>
            <a href="{{ auth()->user()->hasRole('pegawai') ? route('pegawai.show', auth()->user()->pegawai_id) : route('riwayat-skp.index') }}"
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6 md:p-8">
                ...
                <form action="{{ route('riwayat-skp.update', $data->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $data->tahun) }}" min="1990" max="2099" required
                        ...
                        <div>
                            ...
                            <select name="predikat_kinerja" id="predikat_kinerja"
                                ...
                                <option value="Sangat Baik" @selected(old('predikat_kinerja', $data->predikat_kinerja) == 'Sangat Baik')>Sangat Baik</option>
                                <option value="Baik" @selected(old('predikat_kinerja', $data->predikat_kinerja) == 'Baik')>Baik</option>
                                <option value="Butuh Perbaikan" @selected(old('predikat_kinerja', $data->predikat_kinerja) == 'Butuh Perbaikan')>Butuh Perbaikan</option>
                                <option value="Kurang" @selected(old('predikat_kinerja', $data->predikat_kinerja) == 'Kurang')>Kurang</option>
                                <option value="Sangat Kurang" @selected(old('predikat_kinerja', $data->predikat_kinerja) == 'Sangat Kurang')>Sangat Kurang</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pejabat_penilai" id="pejabat_penilai" value="{{ old('pejabat_penilai', $data->pejabat_penilai) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/70 p-5 rounded-xl border border-gray-200">
                        ...
                        <div class="space-y-2">
                            ...
                            @if($data->file_rencana_skp_url)
                                <div class="text-xs text-gray-600">
                                    <a href="{{ $data->file_rencana_skp_url }}" target="_blank" class="text-blue-600 font-semibold hover:underline">
                                        Lihat Dokumen Rencana SKP saat ini
                                    </a>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            @if($data->file_evaluasi_skp_url)
                                <div class="text-xs text-gray-600">
                                    <a href="{{ $data->file_evaluasi_skp_url }}" target="_blank" class="text-emerald-600 font-semibold hover:underline">
                                        Lihat Dokumen Evaluasi SKP saat ini
                                    </a>
                                </div>
                            @endif
                        </div>
