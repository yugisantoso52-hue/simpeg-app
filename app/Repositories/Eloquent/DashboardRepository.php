...
class DashboardRepository implements DashboardRepositoryInterface
{
    ...
    public function getStatistics(): array
    {
        $stats = Pegawai::selectRaw("
            ...
            COALESCE(SUM(CASE WHEN (jenis_pegawai LIKE '%Dosen%' OR (nidn_nuptk IS NOT NULL AND nidn_nuptk != '' AND nidn_nuptk != '-')) AND (jenis_pegawai NOT LIKE '%PPPK%' AND (status_asn = 'ASN' OR LENGTH(nip) = 18)) THEN 1 ELSE 0 END), 0) as dosen_pns,
            COALESCE(SUM(CASE WHEN (jenis_pegawai LIKE '%Dosen%' OR (nidn_nuptk IS NOT NULL AND nidn_nuptk != '' AND nidn_nuptk != '-')) AND (jenis_pegawai LIKE '%PPPK%' OR status_asn = 'PPPK' OR LENGTH(nip) = 21) THEN 1 ELSE 0 END), 0) as dosen_pppk,
