...
@section('title', 'Riwayat Publikasi Ilmiah')
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-gray-500">
                        {{ $item->nama_jurnal ?? $item->penerbit ?? '-' }}
                        @if($item->volume_nomor)
                            <div class="text-xs text-gray-400">Vol. {{ $item->volume_nomor }}</div>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-gray-700 font-medium">{{ $item->tahun_terbit ?? '-' }}</td>
                    <td class="px-4 py-3">
                        @if($item->indeksasi)
                            @php
                                $color = str_starts_with($item->indeksasi, 'Scopus') || str_starts_with($item->indeksasi, 'WoS')
                                    ? 'bg-yellow-100 text-yellow-800'
                                    : (str_starts_with($item->indeksasi, 'SINTA')
                                        ? 'bg-blue-100 text-blue-700'
                                        : 'bg-gray-100 text-gray-600');
                            @endphp
                            <span class="inline-flex items-center rounded-full {{ $color }} px-2 py-0.5 text-xs font-medium">
                                {{ $item->indeksasi }}
                            </span>
                        @else
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
                            @if($item->file_publikasi)
                                <a href="{{ route('document.preview', ['path' => $item->file_publikasi]) }}"
                                   target="_blank" class="text-xs text-green-600 hover:underline">PDF</a>
                            @endif
                            @can('admin')
